...
let template = fs.readFileSync('views/blog/AgenceSeoFrance2026.tsx', 'utf8');
...
let newComponent = template.replace(/AgenceSeoFrance2026/g, 'AgenceSeoEcommerce2026');
newComponent = newComponent.replace(/agence-seo-france-2026/g, 'agence-seo-ecommerce-2026');
newComponent = newComponent.replace(/agence-seo-france/g, 'agence-seo-ecommerce');

newComponent = newComponent.replace(/dangerouslySetInnerHTML=\{\{ __html: \`[\s\S]*?\` \}\}/, `dangerouslySetInnerHTML={{ __html: \`${htmlContent.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\` }}`);
